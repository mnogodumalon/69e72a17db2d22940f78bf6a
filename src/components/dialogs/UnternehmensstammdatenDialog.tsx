...
interface UnternehmensstammdatenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Unternehmensstammdaten['fields']) => Promise<void>;
  defaultValues?: Unternehmensstammdaten['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function UnternehmensstammdatenDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: UnternehmensstammdatenDialogProps) {
  const [fields, setFields] = useState<Partial<Unternehmensstammdaten['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'unternehmensstammdaten');
      await onSubmit(clean as Unternehmensstammdaten['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "unternehmensname": string | null, // Unternehmensname\n  "rechtsform": LookupValue | null, // Rechtsform (select one key: "sonstige" | "gmbh" | "ag" | "gmbh_co_kg" | "kg" | "ohg" | "einzelunternehmen" | "ug") mapping: sonstige=Sonstige, gmbh=GmbH, ag=AG, gmbh_co_kg=GmbH & Co. KG, kg=KG, ohg=OHG, einzelunternehmen=Einzelunternehmen, ug=UG (haftungsbeschränkt)\n  "branche": LookupValue | null, // Branche (select one key: "industrie" | "handel" | "dienstleistung" | "bau" | "it" | "gesundheit" | "transport" | "energie" | "landwirtschaft" | "sonstige_branche") mapping: industrie=Industrie / Produktion, handel=Handel, dienstleistung=Dienstleistung, bau=Bau / Immobilien, it=IT / Technologie, gesundheit=Gesundheit / Pharma, transport=Transport / Logistik, energie=Energie / Versorgung, landwirtschaft=Landwirtschaft, sonstige_branche=Sonstige\n  "handelsregisternummer": string | null, // Handelsregisternummer\n  "steuernummer": string | null, // Steuernummer / USt-IdNr.\n  "strasse": string | null, // Straße\n  "hausnummer": string | null, // Hausnummer\n  "plz": string | null, // Postleitzahl\n  "ort": string | null, // Ort\n  "land": string | null, // Land\n  "ansprechpartner_vorname": string | null, // Vorname Ansprechpartner\n  "ansprechpartner_nachname": string | null, // Nachname Ansprechpartner\n  "ansprechpartner_email": string | null, // E-Mail Ansprechpartner\n  "ansprechpartner_telefon": string | null, // Telefon Ansprechpartner\n  "bemerkungen": string | null, // Bemerkungen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Unternehmensstammdaten['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Unternehmensstammdaten bearbeiten' : 'Unternehmensstammdaten hinzufügen';
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="bau">Bau / Immobilien</SelectItem>
                <SelectItem value="it">IT / Technologie</SelectItem>
                <SelectItem value="gesundheit">Gesundheit / Pharma</SelectItem>
                <SelectItem value="transport">Transport / Logistik</SelectItem>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              value={fields.handelsregisternummer ?? ''}
              onChange={e => setFields(f => ({ ...f, handelsregisternummer: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
