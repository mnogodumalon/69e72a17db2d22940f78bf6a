...
export default function UnternehmensstammdatenPage() {
  const [records, setRecords] = useState<Unternehmensstammdaten[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Unternehmensstammdaten | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Unternehmensstammdaten | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Unternehmensstammdaten | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getUnternehmensstammdaten());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Unternehmensstammdaten['fields']) {
    await LivingAppsService.createUnternehmensstammdatenEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Unternehmensstammdaten['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateUnternehmensstammdatenEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteUnternehmensstammdatenEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} Unternehmensstammdaten im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Unternehmensstammdaten suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('handelsregisternummer')}>
                <span className="inline-flex items-center gap-1">
                  Handelsregisternummer
                  {sortKey === 'handelsregisternummer' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('steuernummer')}>
                <span className="inline-flex items-center gap-1">
                  Steuernummer / USt-IdNr.
                  {sortKey === 'steuernummer' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell>{record.fields.handelsregisternummer ?? '—'}</TableCell>
                <TableCell>{record.fields.steuernummer ?? '—'}</TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={16} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Unternehmensstammdaten. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <ConfirmDialog
        ...
        title="Unternehmensstammdaten löschen"
